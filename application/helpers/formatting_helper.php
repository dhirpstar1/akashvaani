...
function format_address($fields)
{
    ...
    // Default format
    $default = "<strong>{% if company %} {{company}}, {% endif %}{{firstname}} {{lastname}}</strong><br>{{address1}}<br>{% if address2 %}{{address2}}<br>{% endif %}{{city}}, {{zone}} {{zip}}<br>{{country}}</small> <br><small>{{phone}} | {{email}}";
