...
Class Orders extends CI_Model
{
    ...
    {
        $select = 'orders.*'.$this->getAddressSelect().', transactions.transaction_id as transid';
        ...
        \CI::db()->join('customers_address_bank as billing', 'billing.id = orders.billing_address_id', 'left');
		 \CI::db()->join('transactions', 'transactions.order_id = orders.id', 'left');
    ...
    public function getCustomerOrders($id, $limit=0, $offset=0)
    {
		CI::db()->join('transactions', 'transactions.id = orders.transaction_id', 'left');
        CI::db()->order_by('orders.ordered_on', 'DESC');
        CI::db()->where(['orders.customer_id' => $id, 'orders.status !=' => 'cart']);
		 if($limit > 0)
        {
            CI::db()->limit($limit, $offset);
        }
		
		//CI::db()->get('orders')->result();
    ...
    {	CI::db()->join('transactions', 'transactions.id = orders.transaction_id', 'inner');
        CI::db()->where(['orders.customer_id' => $id, 'orders.status !=' => 'cart']);
		//CI::db()->count_all_results('orders');
    ...
    public function getOrder($orderNumber)
    {
		...
		$order->transaction = $this->getTransactionInfo($order->id);

        return $order;
    }
	  function getTransactionInfo($orderId)
		{
			return CI::db()->where('order_id', $orderId)->get('transactions')->row();
		}
