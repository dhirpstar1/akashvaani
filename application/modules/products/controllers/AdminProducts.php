...
class AdminProducts extends Admin {
    ...
    public function form($id = false, $duplicate = false)
    {
		...
		$product_filters = \CI::Products()->get_product_filters($id);	
		
		if($product_filters){
			foreach($product_filters as $filter){
				$data['product_filters'][] = $filter->filter_id;
			}
		}
        }
		...
		$data['add_info']   =  \CI::Products()->get_product_add_info($id);
		$data['reviews']   =  \CI::Products()->get_product_review($id);
        ...
        */
        ...
        else
        {
			...
			$save['product_filters'] = \CI::input()->post('filters');
            // save product
	...
	public function product_addinfo_delete($productid, $id){
	 \CI::Products()->product_addinfo_delete($id);
	 redirect($_SERVER['HTTP_REFERER']);
		//redirect('admin/products/form/'.$productid);
	}
	

 public function product_addinfo($productid, $id)
    {
		...
		if ( \CI::upload()->do_upload())
        {
            ...
            \CI::Products()->update_addinfo($save);
        ...
        $data['error']  = false;
		if($id){
		$data['product_info']  =  \CI::Products()->product_info($id);
		}
		
		
		
		$data['product_id'] = $productid;
        $this->partial('iframe/product_info', $data);
    }
	 ...
	 public function filters()
		{
			...
			$this->view('products_filters', $data);
		}	
		
		public function filter_delete($id)
		{
		 \CI::Products()->delete_filter($id);
	
			redirect('admin/products/filters');
		}	
		
	public function filters_form($id)
		{
		
		
		$filters = \CI::Products()->get_filters('parents');
		foreach($filters as $filter){
		$data['parents'][0] = '-- Select--';
		$data['parents'][$filter->id] = $filter->title;
		}
		if($id){
		$data['filter'] =  \CI::Products()->get_filter($id);
		$data['id']				= $id;
		}
		
		
		
		$data['page_title'] 	= 'Filter Form';
		\CI::form_validation()->set_rules('title', 'Title', 'trim|required');
			
		
        if (\CI::form_validation()->run() == FALSE)
        {
            $this->view('filters_form', $data);
        }
        else
        {
            \CI::load()->helper('text');
            ...
            \CI::Products()->update_filters($save);
	 ...
	 public function category_addinfo($catid, $id)
    {
		...
		if ( \CI::upload()->do_upload())
        {
            ...
            \CI::Products()->update_addinfo($save);
	 		//redirect('admin/categories/form/'.$catid);
			redirect($_SERVER['HTTP_REFERER']);
		}
        ...
        $data['error']  = false;
		if($id){
		$data['category_info']  =  \CI::Products()->product_info($id);
		}

		$data['cat_id'] = $catid;
        $this->partial('iframe/category_info', $data);
    }
	
	 public function product_review($product_id, $id)
    {
	 	\CI::form_validation()->set_rules('review', 'Comments', 'trim|required');
        ...
        if (\CI::form_validation()->run() == TRUE)
        {
			...
			$save['review'] = \CI::input()->post('review');
			$save['customer_name'] = \CI::input()->post('customer_name');
            $save['customer_company'] = \CI::input()->post('customer_company');
        ...
        {
            $upload_data    = \CI::upload()->data();
			$save['customer_image'] = $upload_data['file_name'];
           
		}		
		
		
		
            \CI::Products()->update_review($save);

	 		redirect('admin/products/form/'.$product_id);
		}
		
		if($id){$data['review']  =  \CI::Products()->product_review($id);}
		$data['product_id'] = $product_id;
        $this->partial('iframe/product_review', $data);
    }
	public function product_review_delete($productid, $id){
	 \CI::Products()->product_review_delete($id);
	 redirect($_SERVER['HTTP_REFERER']);
		//redirect('admin/products/form/'.$productid);
	}
