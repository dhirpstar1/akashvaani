...
class Service extends Front {
    ...
    public function get_report_form(){
                $data = array(
                    'name'              => \CI::input()->post('name'),
                    'email'             => \CI::input()->post('email'),
                    'dob'               => \CI::input()->post('dob'),
                    'dot'               => \CI::input()->post('tob'),
                    'location'          => \CI::input()->post('location'),
                    'reference_page'    => \CI::input()->post('reference_page'),
                    'date'              => date('Y-m-d h:i:s')
            		);
           $emaildata['template_id'] 	= '43eb6e3b-ea98-4bde-a52b-89cf14079e38';
           $emaildata['customer_name'] 	= $data['name'];
           $emaildata['email'] 			= $data['email'];
           $emaildata['subject'] 		= 'Thank you to see report.';
          // send_message_form($emaildata);    
          \CI::Service()->save_report_form($data);
		  echo 'saved';
    	exit;
...
// make some dummy data in order to call vedic rishi api
		...
		if(\CI::input()->post('house_type')){ 

		 $data = json_decode($vedicRishi->astroCall("/general_house_report/".\CI::input()->post('house_type'), $data['date'], $data['month'], $data['year'], $data['hour'], $data['minute'], $data['latitude'], $data['longitude'], $data['timezone']));
			
			$data->house_reportArray = explode('.', $data->house_report);
			$house_report_html = '<div>';
			//$house_report_html .= '<p>';
			foreach($data->house_reportArray as $key => $val)
			{
				if($key == 4){
				$house_report_html .= '<a href="javascript:void(0);" class="read_more22" style="">...continue read</a></div><div id="more_text22" style="display: none;">';
				}
				if($val){
				//$house_report_html .= '<p>'.$val.'.</p>';
				
						if($key % 2 == 0){
								$house_report_html .= $val.'.';
						}else{
								$house_report_html .= $val.'.<p></p>';
						}
			
				}
				
				if($val == end($data->house_reportArray)){
				$house_report_html .='<p><a href="javascript:void(0);" class="read_less22">...read less</a></p>';
		...
		//$house_report_html .= '</p>';		
			...
			$house_report_html .= '</div><script>$(\'.read_more22\').click(function(){
	...
	$(\'#more_text22\').show();
		...
		$(\'#more_text22\').hide();
		 ...
		 /* echo substr($data->house_report, 0, 200).'<a href="javascript:void(0);" class="read_more22" style="">...continue read</a><span id="more_text22" style="display: none;"> '.substr($data->house_report, 200).'<a href="javascript:void(0);" class="read_less22">...read less</a></span> */

		 echo $house_report_html; exit;
        ...
        $manglik['manglik'] = json_decode($vedicRishi->astroCall("/manglik/", $data['date'], $data['month'], $data['year'], $data['hour'], $data['minute'], $data['latitude'], $data['longitude'], $data['timezone']));
		 ...
		 //$manglik['general_house_report_jupiter'] = json_decode($vedicRishi->astroCall("/general_house_report/Jupiter", $data['date'], $data['month'], $data['year'], $data['hour'], $data['minute'], $data['latitude'], $data['longitude'], $data['timezone']));
		...
		$general_ascendant_report_data = json_decode($vedicRishi->astroCall("/general_ascendant_report/", $data['date'], $data['month'], $data['year'], $data['hour'], $data['minute'], $data['latitude'], $data['longitude'], $data['timezone']));
			...
			$manglik['ascendant'] = $general_ascendant_report_data->asc_report->ascendant;
	...
	$(\'#more_text21\').show();
		...
		$manglik['general_ascendant_report'] = $general_ascendant_report;
...
// make some dummy data in order to call vedic rishi api
        ...
        $general_ascendant_report_data = json_decode($vedicRishi->astroCall("/general_ascendant_report/", $data['date'], $data['month'], $data['year'], $data['hour'], $data['minute'], $data['latitude'], $data['longitude'], $data['timezone']));
			...
			$child_pregnancy['ascendant'] = $general_ascendant_report_data->asc_report->ascendant;
	...
	$(\'#more_text21\').show();
		...
		$child_pregnancy['cccode']                   = (\CI::session()->userdata('country_code')) ? \CI::session()->userdata('country_code') : 'IN';		
		$child_pregnancy['general_ascendant_report'] = $general_ascendant_report;
...
// make some dummy data in order to call vedic rishi api
  ...
  $general_ascendant_report_data = json_decode($vedicRishi->astroCall("/general_ascendant_report/", $data['date'], $data['month'], $data['year'], $data['hour'], $data['minute'], $data['latitude'], $data['longitude'], $data['timezone']));
			...
			$career['ascendant'] = $general_ascendant_report_data->asc_report->ascendant;
	...
	$(\'#more_text21\').show();
		...
		$career['general_ascendant_report'] = $general_ascendant_report;
...
// make some dummy data in order to call vedic rishi api
	...
	public function booking_question(){
		   ...
		   error_reporting(0);
 $cccode   = (\CI::session()->userdata('country_code')) ? \CI::session()->userdata('country_code'):'IN';
		   $logged_customer	= \CI::Login()->customer();
                   $report_type         = \CI::input()->post('report_type');
                   $question_type       = \CI::input()->post('question_type');
...
//print_r($data); exit;
			...
			foreach($html->find('question') as $key =>  $element) 
				 ...
				 { 			 
				    ...
				    if($count == 0){ 	
						$addquestion['price'] 			= get_key_data('PER_QUESTION_PRICE', $cccode, false);
					}else{
						$addquestion['price'] 			= get_key_data('OTHER_QUESTION_PRICE', $cccode, false);
					}
					$emaildata['questions'][] 	= $addquestion;
					
					if($count == 0){ 
						$amount += get_key_data('PER_QUESTION_PRICE', $cccode, false);
					}else{
						$amount += get_key_data('OTHER_QUESTION_PRICE', $cccode, false);
					}
					$count++;
				 }
...
//print_r($emaildata['questions']); exit;	
				...
				$emaildata['free_question'] = false;
                        ...
                        if($question_type == 3){   
                         $emaildata['question'] =  $emaildata['special'] = 'Special offer with 3 Questions';

                         $emaildata['price']    =  $amount = get_key_data('SET_OF_THREE_QUESTIONS', $cccode, false);
                            array_unshift($emaildata['questions'], $emaildata);     
				$emaildata['free_question'] = false;
                           
                        }elseif($question_type == 5){
                          $emaildata['question'] =  $emaildata['special'] = 'Special offer with 5 Questions';
                          $emaildata['price']    =  $amount = get_key_data('SET_OF_FIVE_QUESTIONS', $cccode, false);
                            array_unshift($emaildata['questions'], $emaildata);   
				          $emaildata['free_question'] = false;
                        }
                        elseif($question_type == 6){
                          $emaildata['question'] =  $emaildata['special'] = '2018 Marriage, health, career & Life report ';
                          $emaildata['price']    =  $amount = get_key_data('2018_MARRIAGE_HEALTH_CAREER_LIFE_REPORT', $cccode, false);
                            array_unshift($emaildata['questions'], $emaildata);   
                          $emaildata['free_question'] = false;
                        }
                        elseif($question_type == 7){
                          $emaildata['question'] =  $emaildata['special'] = 'All About your Career in 2018';
                          $emaildata['price']    =  $amount = get_key_data('ALL_ABOUT_YOUR_CAREER_IN_2018', $cccode, false);
                            array_unshift($emaildata['questions'], $emaildata);   
                          $emaildata['free_question'] = false;
                        }
                        elseif($question_type == 8){
                          $emaildata['question'] =  $emaildata['special'] = 'Your Love & Marriage in 2018 ';
                          $emaildata['price']    =  $amount = get_key_data('ALL_ABOUT_YOUR_CAREER_IN_2018', $cccode, false);
                            array_unshift($emaildata['questions'], $emaildata);   
                          $emaildata['free_question'] = false;
                        } elseif($question_type == 9){
                          $emaildata['question'] =  $emaildata['special'] = 'Marriage compatibility in 2018 ';
                          $emaildata['price']    =  $amount = get_key_data('ALL_ABOUT_YOUR_CAREER_IN_2018', $cccode, false);
                            array_unshift($emaildata['questions'], $emaildata);   
                          $emaildata['free_question'] = false;
                        }
			 ...
			 if($report_type == 1){     
                            $emaildata['question'] =  $emaildata['special'] = 'Special Commentary - The story of my Life report (70 Pages)';
                            $emaildata['price']    =  $amount = get_key_data('THE_STORY_OF_YOUR_LIFE_SPECIAL_COMMENTARY', $cccode, false);
                            array_unshift($emaildata['questions'], $emaildata);
				$emaildata['free_question'] = false;
                        }elseif($report_type == 2){
                           $emaildata['question'] =  $emaildata['special'] = 'Special Commentary - The story of my Life report (25 Pages)';
                           $emaildata['price']    =  $amount = get_key_data('THE_STORY_OF_YOUR_LIFE', $cccode, false);
                           array_unshift($emaildata['questions'], $emaildata);
				$emaildata['free_question'] = false;
                              }elseif($report_type == 3){
                           $emaildata['question'] =  $emaildata['special'] = 'Special Commentary - Kundali Matching Report';
                           $emaildata['price']    =  $amount = get_key_data('KUNDALI_MATCHING_SPECIAL_COMMENTARY', $cccode, false);
                           array_unshift($emaildata['questions'], $emaildata);
                $emaildata['free_question'] = false;
                              }elseif($report_type == 5){
                           $emaildata['question'] =  $emaildata['special'] = 'All about your Marriage, Career & Health in 2018';
                           $emaildata['price']    =  $amount = get_key_data('ALL_ABOUT_YOUR_MARRIAGE_CAREER_HEALTH_IN_2018', $cccode, false);
                           array_unshift($emaildata['questions'], $emaildata);
                $emaildata['free_question'] = false;
                ...
                $emaildata['free_question'] = false;
                           ...
                           $emaildata['question'] =  $emaildata['special'] = 'All About your Health in 2018';
                           $emaildata['price']    =  $amount = get_key_data('ALL_ABOUT_YOUR_CAREER_IN_2018', $cccode, false);
                           array_unshift($emaildata['questions'], $emaildata);
                $emaildata['free_question'] = false;
                              }elseif($report_type == 11){
                           $emaildata['question'] =  $emaildata['special'] = 'Your Money & Finance in 2018';
                           $emaildata['price']    =  $amount = get_key_data('ALL_ABOUT_YOUR_CAREER_IN_2018', $cccode, false);
                           array_unshift($emaildata['questions'], $emaildata);
                $emaildata['free_question'] = false;
                              }elseif($report_type == 12){
                           $emaildata['question'] =  $emaildata['special'] = 'Your Business in 2018';
                           $emaildata['price']    =  $amount = get_key_data('ALL_ABOUT_YOUR_CAREER_IN_2018', $cccode, false);
                           array_unshift($emaildata['questions'], $emaildata);
                $emaildata['free_question'] = false;
                              }
			...
			//$amount = 1.50;
                        ...
                        //\CI::Service()->sendPaymentNotification($optionsdata['booking_id'], $emaildata);	
	...
	public function process_payment($booking_id){
			...
			$questiondata 							= \CI::Service()->get_question_by_booking_id($booking_id);
			$data['questions']						= \CI::Service()->get_questions_by_booking_id($booking_id);
			$merchant_data='';
			...
			//print_r($data); exit;
				...
				$this->partial('_blocks/process_payment', $data);
	...
	public function booking_full_consultation(){
		   ...
		   switch(\CI::input()->post('consultation_type')){
					...
					$amount = get_key_data('KUNDALI_MATCHING', $cccode, false);
					...
					$amount = get_key_data('FULL_COUNSULT', $cccode, false);
					...
					$amount = get_key_data('YOUR_LIFETIME_GUIDE', $cccode, false);
                    ...
                    $amount = get_key_data('KNOW_YOUR_SELF', $cccode, false);
                    ...
                    $amount = get_key_data('THE_STORY_OF_YOUR_LIFE', $cccode, false);
...
<meta name="description" content="Know About Your Yearly Pisces Horoscope, Akashvaani.com provides Online Yearly Pisces Horoscope Prediction, Sign Up Today for Personal Yearly Horoscope Matching by Experience Astrologers in India." />';
	...
	public function copatibilityresult($slug=NULL){
	 ...
	 public function daily_reminder($slug ='notpaid')
		{

			\CI::load()->model(array('Service'));
			$data = \CI::Service()->daily_reminder($slug);	
			$count = 0;


				foreach($data as $booking){

					$senddata['template_id'] = '7b74f2d9-393b-4203-863b-d9fad9af31e5';
					$senddata['customer_name'] = $booking->first_name;
					$senddata['body'] = '';
					$senddata['payment_link'] = '<a href="'.base_url('/process_payment/'.$booking->booking_id).'">'.$booking->question.'</a>';
					$senddata['email'] = $booking->email;
