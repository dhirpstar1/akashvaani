...
Class Service extends CI_Model
{
	...
	function sendPaymentNotification($booking_id, $data){		
		...
		GoCart\Emails::sendPaymentNotification($result, 20, $invoice);
			if($result['phone']){
		...
		GoCart\Emails::sendPaymentNotification($result, 15, $invoice);
	...
	function get_questions($id=0){
		...
		if($ordid > 0){
			$result = CI::db()->get_where('question_category', array('question_category.status'=>1, 'question_category.id' => $ordid))->result();
			 ...
			 $result = CI::db()->get_where('question_category', array('question_category.status'=>1))->result();
		}
               // 
