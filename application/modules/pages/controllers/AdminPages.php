...
class AdminPages extends Admin
{
 ...
 \CI::load()->helper('text');
            ...
            $data['options'] = \CI::input()->post('options');

            if($data['options']){
                    foreach (array_reverse($data['options']) as $key => $value) {
                        $dataInsert['page_id'] = $data['id'];
                        $dataInsert['blog_id'] = $value;
                        $dataInsert['order_id'] = $key+1;
                        $Inserdata[] = $dataInsert;
                     }
            }
\CI::Pages()->update_save_articles($Inserdata, $data['id']);



echo json_encode($Inserdata); exit;
    }
    ...
    ********************************************************************/
    function form($id = false)
    {
        ...
        {	
			if($id){
                 $data['articles'] = \CI::Blogs()->articles_blogs($id);
            }
