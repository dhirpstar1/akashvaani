...
Class Customers extends CI_Model
{
	 ...
	 public function validate_otp()
    {
		...
		CI::db()->select('id');
		CI::db()->where(array('customer_email' => trim(\CI::input()->post('email')), 'otp' => trim(\CI::input()->post('otp'))));
    ...
    public function questions_send_email($type, $id) {
                     CI::db()->select('customer_questions.*, module_bookings.first_name, module_bookings.last_name, module_bookings.email, module_bookings.phone, module_bookings.date');
                    CI::db()->join('module_bookings', 'customer_questions.booking_id = module_bookings.id', 'left');
                    CI::db()->from('customer_questions');
                    CI::db()->where('customer_questions.id', $id);
                ...
                switch($type){
                    ...
                    case 'deleverd':
                    $senddata['template_id'] = 'fdfbcb96-60a9-4b86-9df1-c860ecc3e17d';
                    $senddata['subject'] = 'Order delivered by akshavaani.com';

                   
            CI::db()->select('customer_questions.*, SUM(customer_questions.amount) as total_amount, module_bookings.first_name, module_bookings.last_name, module_bookings.email, module_bookings.gender, module_bookings.phone');
        CI::db()->join('module_bookings', 'module_bookings.id = customer_questions.booking_id');
        $resultdata = CI::db()->get_where('customer_questions', array('customer_questions.booking_id' => $result->booking_id))->row_array();
    
        CI::db()->select('customer_questions.*');
        //CI::db()->join('module_bookings', 'customer_questions.booking_id = module_bookings.id');
        $emaildata['questions'] = CI::db()->get_where('customer_questions', array('customer_questions.booking_id' => $result->booking_id))->result_array(); 
            CI::db()->select('module_bookings_options.*');
        $emaildata['booking_options'] = CI::db()->get_where('module_bookings_options', array('module_bookings_options.booking_id' => $result->booking_id))->row_array();    
                $emaildata['data'] = $resultdata;

//print_r($emaildata); exit;

                                    $senddata['email_body'] = CI::load()->view("_blocks/invoice_details", $emaildata, true);
                                    $senddata['booking_id'] = $result->booking_id;
                    break; 

                    case 'feedback':
                    $senddata['template_id'] = '4816c13d-dbd4-4b77-8b34-93643205aa1c';
                    $senddata['subject'] = 'FEEDBACK - akashvaani.com';
                    break;                         
                   ...
                   return  send_message_form($senddata);    
    ...
    public function get_report_enquiries($count = false, $limit=0, $offset=0, $order_by='id', $direction='DESC', $reference_by){
           CI::db()->select('get_analysis_data.*');
           if($reference_by !== 'all'){
           // CI::db()->where('get_analysis_data.reference_page', $reference_by);
           }
          // CI::db()->where('module_bookings.email !=', '');
         //CI::db()->join('customers', 'customer_questions.customer_id = customers.id', 'left');
        if($count){
        ...
        $result = CI::db()->get('get_analysis_data');
        return $result->result();
        }
    }
...
\CI::TCPDF()->lastPage();
	...
	public function send_payment_reminder($link, $data){
                    ...
                    $senddata['customer_name']  = $data['question']->first_name;
                    $senddata['payment_link']   = $link;
                    $senddata['email']          = $data['question']->email;
