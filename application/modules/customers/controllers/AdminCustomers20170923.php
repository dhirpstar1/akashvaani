...
class AdminCustomers extends Admin {
	...
	public function questions_get_reports($field='id', $by='DESC', $page=0)
    {
	     
        $data['page_title'] = 'Customer Get Reports';
        $data['questions'] = \CI::Customers()->get_reports_questions(50, $page, $field, $by);

        \CI::load()->library('pagination');

        $config['base_url'] = site_url('/admin/customers/questions_get_reports/'.$field.'/'.$by.'/');
        $config['total_rows'] = \CI::Customers()->count_get_reports_questions();
	...
	public function download_report($id = false)
    {   if ($id)
        { 
			...
			$report_path = FCPATH.'uploads/reports/report_'.$id.'.pdf';	
			file_put_contents($report_path, file_get_contents($apidata->pdf_url));		
			
			$file = \CI::Customers()->create_pdf($report_path, $mdata, true);
			exit;
	}
		
	}		
	public function send_report($id = false)
    {
        if ($id)
        { 
            $question = \CI::Customers()->get_report_question($id);		
			...
			$file = \CI::Customers()->create_pdf($report_path, $mdata, $doenload);
			...
			$updatequestion['answer_link'] 	= base_url('uploads/reports/report_'.$id.'.pdf');	
