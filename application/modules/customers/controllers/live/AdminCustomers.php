...
public function report_enquiries($reference_by = 'all',$field = 'id', $by='DESC', $page=0)
    {
        ...
        $data['page_title']         = 'Customer Questions';
        $data['enquiries']          = \CI::Customers()->get_report_enquiries(false, 50, $page, $field, $by, $reference_by);

        \CI::load()->library('pagination');

        $config['base_url']         = site_url('/admin/customers/report_enquiries/'.$reference_by.'/'.$field.'/'.$by.'/');
        $config['total_rows']       = \CI::Customers()->get_report_enquiries(true);
        ...
        $data['references']         = array('all' => '-- All --',
                                            'marriage-kundali-matching' => 'Kundali Milan', 
                                            'online-marriage-prediction' => 'Marriage prediction', 
                                            'manglik-dosha-solution' => 'Manglik Dosh', 
