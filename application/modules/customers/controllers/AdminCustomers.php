...
class AdminCustomers extends Admin {
    ...
    public function questions_send_email($type = 'delay', $questionid = 0)
    {
        
       echo \CI::Customers()->questions_send_email($type, $questionid);
    }
...
public function report_enquiries($reference_by = 'all',$field = 'id', $by='DESC', $page=0)
    {
        ...
        $data['page_title']         = 'Customer Questions';
        $data['enquiries']          = \CI::Customers()->get_report_enquiries(false, $reference_by, 50, $page, $field, $by);

        \CI::load()->library('pagination');

        $config['base_url']         = site_url('/admin/customers/report_enquiries/'.$reference_by.'/'.$field.'/'.$by.'/');
        $config['total_rows']       = \CI::Customers()->get_report_enquiries(true, $reference_by);
        ...
        $data['field']              = $field;
        $data['by']                 = $by;
        $data['selected_reference'] = $reference_by;
        $data['references']         = array('all' => '-- All --',
                                            'manglik-kundali-matching' => 'Kundali Milan', 
                                            'online-marrige-prediction' => 'Marriage prediction', 
                                            'manglik-kundali-matching' => 'Manglik Dosh', 
                                            'child-pregnancy' => 'Child and Pregnancy', 
                                            'love-marriage-relationships' => 'Love Marriage', 
                                            'Health' => 'Medical and Health', 
                                            'Career' => 'Career and Job'); 
        $this->view('report_enquiries', $data);
    }
...
//print_r($question); exit;
            ...
            $data['customer_id'] 	= $question['question']->customer_id;
            $data['booking_id'] 	= $question['question']->booking_id;
            $data['question'] 		= $question['question']->question;
            $data['answer'] 		= $question['question']->answer;
            $data['answer_link'] 	= $question['question']->answer_link;
    ...
    {   if ($id)
        { 
			...
			if(!$apidata->status || empty($apidata)){		
	 ...
	 public function report_form($id = false)
    {
        ...
        if ($id)
        { 
            $data = \CI::Customers()->get_report_form($id);
	...
	public function send_payment_reminder($queid){
		...
		$link = base_url('questions_payment/'.$data['question']->booking_id);
				
		echo \CI::Customers()->send_payment_reminder($link, $data);
		exit;
	}
