...
class Locations extends CI_Model 
{
    ...
    public function save_country($data)
    {
     ...
     //countries
    public function save_constant($data)
    {
        if(!$data['id']) 
        {
            CI::db()->insert('constants', $data);
            return CI::db()->insert_id();
        } 
        else 
        {
            CI::db()->where('id', $data['id']);
            CI::db()->update('constants', $data);
            return $data['id'];
        }
    }
 ...
 public function get_constants()
    {
        return CI::db()->order_by('sequence', 'ASC')->get('constants')->result();
    }
    ...
    {
        CI::db()->where('id', $id);
        return CI::db()->get('constants')->row();
    }
    public function delete_constant($id)
    {
        CI::db()->where('id', $id);
        CI::db()->delete('constants');
    }
