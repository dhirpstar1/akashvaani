...
class AdminLocations extends Admin {
    ...
    {
        $data['constants']  = \CI::Locations()->get_constants();
        
        $this->view('constants', $data);
    }
 ...
 public function constant_form($id = false)
    {
        \CI::load()->helper('form');
        \CI::load()->library('form_validation');
        ...
        $data['constant_key'] = '';
        $data['constant_value'] = '';
        $data['constant_optional_value'] = '';
        $data['constant_lng'] = '';
        $data['constant_location'] = '';

        if ($id)
        {   
            $data = (array)\CI::Locations()->get_constant($id);
        ...
        \CI::form_validation()->set_rules('constant_key', 'Key', 'trim|required');
        \CI::form_validation()->set_rules('constant_value', 'Value', 'trim|required');
        \CI::form_validation()->set_rules('constant_location', 'Location', 'trim');      
    
        if (\CI::form_validation()->run() == FALSE)
        {
            $this->view('constant_form', $data);
        }
        else
        {
            $save['id']                             = $id;
            $save['constant_key']                   = \CI::input()->post('constant_key');
            $save['constant_value']                 = \CI::input()->post('constant_value');
            $save['constant_optional_value']        = \CI::input()->post('constant_optional_value');
            $save['constant_lng']                   = 'en';
            $save['constant_location']              = \CI::input()->post('constant_location');

            \CI::Locations()->save_constant($save);
            
            \CI::session()->set_flashdata('message', 'Constant has been saved.');
            
            //go back to the product list
            redirect('admin/locations/constants');
        }
    }
    
    public function delete_constant($id = false)
    {
        if ($id)
        {   
            $location = \CI::Locations()->get_constant($id);
            //if the promo does not exist, redirect them to the customer list with an error
            if (!$location)
            {
                \CI::session()->set_flashdata('error', 'Constant not found.');
                redirect('admin/locations/constants');
            }
            else
            {
                \CI::Locations()->delete_constant($id);
                
                \CI::session()->set_flashdata('message', 'Delete Constant successfully.');
                redirect('admin/locations/constants');
            }
        }
        else
