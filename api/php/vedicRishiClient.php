...
class VedicRishiClient
{
    ...
    {
		$this->userId = ($uid) ? $uid : USER_ID;
        $this->apiKey = ($key) ? $key : API_KEY;	
    }
 ...
 public function pdfCall($resourceName, $data)
    {
        $resData = $this->getPdfCurlReponse($resourceName, $data);
        return $resData;

    }

 public function gethoroscopedetail($type,$slug)
    {
        $serviceUrl = 'http://horoscope-api.herokuapp.com/horoscope/'.$type.'/'.$slug;
        $ch = curl_init();
        curl_setopt($ch,CURLOPT_URL, $serviceUrl);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_TIMEOUT, 20);
	 ...
	 public function gethoroscope($type,$slug)
    {
        $serviceUrl = 'http://sandipbgt.com/theastrologer/api/horoscope/'.$slug.'/'.$type;
          $options = array(
