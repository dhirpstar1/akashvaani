    ...
    <div class="row">
      ...
      <div class="col-md-12" align="center"><h3>Ask Question</h3></div>
        ...
        <p class="payment_head">Payment Summary</p>
                 ...
                 <?php print_r($data); exit; if($cccode == 'US'){ ?>
 <a href="<?=base_url('/service/pay_now/'.$optionsdata['booking_id']);?>" type="button" class="btn btn-primary paynowpaypalbutton waves-effect waves-light float-xs-right" >Pay Now</a>
          ...
          <div class="col-xs-6">
           ...
           <div class="col-xs-6">
            <address class="float-xs-right">
              <p class="price_text">Total - <?=get_currency();?><?php echo $data['total_amount']; ?>/-</p>
            </address>
          </div>
        </div>
             ...
             <?php foreach ($questions as $key => $question): //print_r($question);?>                                     
                  <tr>
                    <td>
                     ...
                     <td></td>
                    <td class="text-right"><?=get_currency();?> <?php echo number_format($question['amount'], 2); ?></td>
                  </tr>
              <?php endforeach; ?>               
                  <tr>
                    <td class="thick-line"></td>
                    <td class="thick-line text-center"><strong>Subtotal</strong></td>
                    <td class="thick-line text-right"><?=get_currency();?><?php echo $data['total_amount']; ?></td>
                  </tr>
                  <tr>
                    <td class="no-line"></td>
                    <td class="no-line text-center"><strong>Total</strong></td>
                    <td class="no-line text-right"><?=get_currency();?><?php echo $data['total_amount']; ?></td>
                  </tr>
                </tbody>
              </table>
            </div>
